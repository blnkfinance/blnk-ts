...
import { ApiResponse, ApiResponseError, FormatResponseType, ServiceInstances, ServicesMap } from "../../types/general";
...
export class Blnk {
    ...
    constructor(apiKey: string, options: BlnkClientOptions, services: ServicesMap, formatResponse: FormatResponseType) {
        ...
        this.apiKey = apiKey;
        let { logger, ...restOptions } = options
        this.options = {
    ...
    */
    private async request<T, R>(endpoint: string, data: T, method: 'POST' | 'GET' | 'PUT' | 'DELETE'): Promise<ApiResponse<R | null>> {
        const headers = {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                this.logger.error(`Request to ${endpoint} failed with status ${response.status}.`);
                return this.formatResponse<R>(response.status, response.statusText, errorResult);
            }

            const jsonResponse = await response.json() as R;
            return this.formatResponse<R>(response.status, "Success", jsonResponse) as ApiResponse<R>;
        } catch (error: any) {
    ...
    private getService<T>(serviceName: string): T {
        ...
        if (!this.serviceInstances[serviceName]) {
            this.serviceInstances[serviceName] = new this.services[serviceName](this.request.bind(this), this.logger, this.formatResponse);
        }
